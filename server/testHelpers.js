...
const setPassword = function(line, socket, rooms, roomsSeen, playerRoom) {
  for (const room of rooms) {
    if (line.startsWith(`'User ${room} with bot AI "screeps-bot" spawned in ${room}'`)) {
      roomsSeen[room] = true;
...
async function initServer() {
  ...
  .replace("runner_threads = 4", "runner_threads = 8")
...
const spawnBots = async function(line, socket, rooms, players, tickDuration) {
  if (line.startsWith(`Screeps server v`)) {
    ...
    for (const room of rooms) {
      ...
      socket.write(`bots.spawn('screeps-bot', '${room}', {username: '${room}', cpu: 100, gcl: 1, x: ${players[room].x}, y: ${players[room].y}})\r\n`);
      await sleep(1);
    }
    return true;
  }
